...
# 当前脚本版本号
VERSION='2.2.0-pqc-strong (2026.07.02, strict ML-KEM hybrid VLESS Encryption)'
...
normalize_vless_pqc_ticket() {
  local _v="$1" _resume="${VLESS_PQC_RESUME:-600s}"
  if truthy "${VLESS_PQC_DISABLE_0RTT:-y}"; then
    printf '%s' "$_v" | sed -E "s/^(mlkem768x25519plus\.[^.]+\.)(0rtt|[0-9]+s|[0-9]+-[0-9]+s)\./\1${_resume}./"
  else
    printf '%s' "$_v"
  fi
}
...
vless_pqc_runtime_values() {
  ...
  if truthy "$ENABLE_VLESS_PQC" && [ -n "$VLESS_PQC_DECRYPTION" ] && [ -n "$VLESS_PQC_ENCRYPTION" ]; then
    VLESS_PQC_DECRYPTION=$(normalize_vless_pqc_ticket "$VLESS_PQC_DECRYPTION")
    VLESS_PQC_ENCRYPTION=$(normalize_vless_pqc_ticket "$VLESS_PQC_ENCRYPTION")
...
fetch_nodes_value() {
  unset SERVER_IP REALITY_PORT REALITY_PUBLIC REALITY_PRIVATE TLS_SERVER SERVER SERVER_PORT SERVER_DISPLAY UUID WS_PATH NODE_NAME SS_METHOD SS2022_PASSWORD \
        ENABLE_VLESS_PQC VLESS_PQC_STRICT VLESS_PQC_DECRYPTION VLESS_PQC_ENCRYPTION VLESS_SERVER_DECRYPTION VLESS_CLIENT_ENCRYPTION VLESS_CLIENT_ENCRYPTION_QUERY \
  ...
  VLESS_PQC_DECRYPTION="${vlessPqcDecryption:-${VLESS_PQC_DECRYPTION:-}}"
  VLESS_PQC_ENCRYPTION="${vlessPqcEncryption:-${VLESS_PQC_ENCRYPTION:-}}"
  unset serverIp privateKey publicKey cdn cdnPort language enableVlessPqc vlessPqcStrict vlessPqcDecryption vlessPqcEncryption
...
' ' ')
  ...
  grep -q 'reality-vision' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${NODE_TAG[0]}\", type: vless, server: ${SERVER_IP}, port: ${REALITY_PORT}, uuid: ${UUID}, encryption: "${VLESS_CLIENT_ENCRYPTION:-none}", network: tcp, udp: true, tls: true, servername: ${TLS_SERVER}, flow: xtls-rprx-vision, client-fingerprint: chrome, reality-opts: {public-key: ${REALITY_PUBLIC}, short-id: \"\"} }" \
    ...
    "vless://${UUID}@${SERVER_IP_1}:${REALITY_PORT}?encryption=${VLESS_CLIENT_ENCRYPTION_QUERY:-none}&flow=xtls-rprx-vision&security=reality&sni=${TLS_SERVER}&fp=chrome&pbk=${REALITY_PUBLIC}&type=tcp&headerType=none#${NODE_NAME// /%20}%20${NODE_TAG[0]}" \
  ...
  grep -q 'reality-grpc' <<< "$PROTOS_NOW" && _add \
    ...
    "vless://${UUID}@${SERVER_IP_1}:${GRPC_PORT}?security=reality&sni=${TLS_SERVER}&fp=chrome&pbk=${REALITY_PUBLIC}&type=grpc&serviceName=grpc&encryption=${VLESS_CLIENT_ENCRYPTION_QUERY:-none}#${NODE_NAME// /%20}%20${NODE_TAG[2]}" \
